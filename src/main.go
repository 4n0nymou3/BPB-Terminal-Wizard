...
func main() {
    ...
    for attempt := 1; attempt <= 3; attempt++ {
        ...
        if err != nil {
            message := fmt.Sprintf("Error getting KV ID! Output: %s. Check logs at ~/.config/.wrangler/logs/ for details. Ensure Wrangler is version 4.12.0 or higher.", output)
            failMessage(message, err)
            continue
        }
...
func checkNode() error {
    ...
    if !strings.HasPrefix(version, "v18") && !strings.HasPrefix(version, "v20") {
